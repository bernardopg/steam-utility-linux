...
| `idle` | `<app_id> [app_name]` | `<app_id> [app_id2 ...] [app_name]` | Single-app mode preserves upstream behavior; multi-app mode is a local enhancement that starts one child idle process per AppID because Steamworks only supports one AppID per process | Invalid app id, missing installation, success payload, optional app name, init failure, and multi-app dispatch are regression-tested; Linux live validation exists | Yes | deterministic CLI tests added + fake-backed native seam + Linux real-environment validation completed + Windows real-environment validation pending | Extend toward Windows real-session validation |
